...
/* Statistik dari riwayat tersimpan, dibatasi ke pair+timeframe yang sedang aktif
   (win rate XAU/USD M15 tidak relevan dicampur dengan EUR/USD H1). */
function getPersistedStats(windowN){
  ...
  const wins = windowed.filter(h=>h.realizedR>0).length;
  return {
    total: rel.length,
    windowCount: windowed.length,
    winRate: windowed.length ? wins/windowed.length*100 : null,
    avgR: windowed.length ? windowed.reduce((a,h)=>a+h.realizedR,0)/windowed.length : null,
  };
}
...
/* ═══════════════════════════════════════════════════════════
   ...
   ambang disesuaikan naik/turun mengikuti win rate 30 trade terakhir:
   win rate rendah → makin selektif (ambang naik), win rate tinggi → sedikit
   lebih longgar (tangkap lebih banyak sinyal selagi strategi sedang akurat). */
function computeAdaptiveThreshold(maxScoreRef){
  ...
  let pct = BASE_PCT;
  if(stats.winRate < 40) pct = 0.62;
  else if(stats.winRate < 50) pct = 0.55;
  else if(stats.winRate < 58) pct = BASE_PCT;
  else pct = 0.38;
  return { pct, score:pct*maxScoreRef, reason:`WR ${stats.windowCount} trade terakhir: ${stats.winRate.toFixed(0)}%` };
}
...
function computeEngine(candles, cfg, mtfCandles){
  ...
  if(cfg.useBos) maxScoreRef += CONST.MAX_BOS_SCORE;
  if(cfg.useRsiDivergence) maxScoreRef += CONST.MAX_DIVERGENCE_SCORE;
  if(cfg.useMtf && mtfAvailable) maxScoreRef += CONST.MAX_MTF_SCORE;
  ...
  function scoreBreakdown(i,isBuy){
    ...
    const mtfScore = (!cfg.useMtf || !mtfAvailable) ? 0
      : (mtfDir==null ? CONST.MAX_MTF_SCORE*0.5 : (mtfDir===(isBuy?1:-1) ? CONST.MAX_MTF_SCORE : 0));
    ...
    const bosScore = !bosEvent ? 0 : (bosEvent.startsWith('CHOCH') ? CONST.MAX_BOS_SCORE : CONST.MAX_BOS_SCORE*0.6);

    // ── Divergence RSI vs harga
    const divPattern = detectRsiDivergence(i,isBuy);
    const divScore = divPattern ? CONST.MAX_DIVERGENCE_SCORE : 0;
      ...
      if(emaOkBuy){
        ...
        if(sb.total >= entryThreshold.score){
          ...
          skippedSignals.push({i, time:candles[i].time, side:'BUY', price:closes[i], score:sb.total, tqi:tqi[i],
            pattern: sb.patternNames.join(' + ') || null, status:'SKIPPED', valid:false});
        }
      }
...
async function fetchCandles(){
  return fetchCandlesRaw(state.symbol, state.interval, state.outputSize, state.apiKey);
}
...
async function fetchHtfCandles(){
  ...
  try{
    const candles = await fetchCandlesRaw(state.symbol, hi, Math.min(state.outputSize, 300), state.apiKey);
...
function renderAll(res, candles){
  ...
  document.getElementById('sPersistedWR').title = `${pStats.windowCount} trade tersimpan terakhir (dari total ${pStats.total} tersimpan) untuk ${state.symbol} ${state.interval}.`;
...
document.getElementById('resetHistoryBtn').onclick = ()=>{
  if(!confirm(`Hapus semua riwayat tersimpan (${persistedHistory.length} trade)? Ini akan mengembalikan ambang skor adaptif ke baseline.`)) return;
